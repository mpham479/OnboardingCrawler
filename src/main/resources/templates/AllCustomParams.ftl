<html>
	...
	<head>
	  <title>Custom Param List</title>
	</head>
	<body>
	  <span class="h1copy">Custom Param List</span>
	  ...
	  <div class="tableBlock">
		  <table class="data" id="data">
		  	<thead>
				<tr>
					...
					<th class="colHeader noSelect clickableHeader imgHover" onclick="sortTable(1)" sortdir="" data="Type">Type <img src="" class="imgOn" style='width:14px'><img src="" class="imgOff" style='width:14px'></th>
					<th class="colHeader noSelect clickableHeader imgHover" onclick="sortTable(2)" sortdir="" data="Description">Description <img src="" class="imgOn" style='width:14px'><img src="" class="imgOff" style='width:14px'></th>
					<th class="colHeader"></th>
				</tr>
			</thead>
			<tbody>
			<#foreach customparam in customparams?values>
				<#if customparam.name??>                  
				<tr class='${["odd", "even"][customparam_index%2]}' systemid='${customparam.name}'>
					...
					<td class="default" style="white-space:normal;">${(customfield.description)!" "}</td>
					<td class="moreInfo">
						<#if fileBasedLinks>
		...
		function openModalInfo(ele){
			...
			frameUrl = "file:customparams/" + ele.parentElement.parentElement.parentElement.getAttribute("systemid") + ".${baseExt}"
		  	...
		  	<#else>
		  		<#if usedInClickHelp>
		  			frameUrl = "${articleBaseUrl}" + ele.parentElement.parentElement.parentElement.getAttribute("systemid").replace(/_/g,'${replaceSpacesInUrlsWith}').replace(/\./g, '-');
			  		modal.style.display = "block"
