<html>
	...
	<body>
	  <span class="h1copy">Script List</span>
	  ...
	  <div class="tableBlock">
		  <table class="data" id="data">
		  	<thead>
				<tr>
					...
					<th class="colHeader noSelect clickableHeader imgHover" onclick="sortTable(2)" sortdir="" data="Script Type">Script Type <img src="" class="imgOn" style='width:14px'><img src="" class="imgOff" style='width:14px'></th>
			...
			<#foreach script in scripts?values>                  
				<tr class='${["odd", "even"][script_index%2]}' systemid='${script.systemId}'>
					<td class="default">
						<#if fileBasedLinks>
							<a href="file:scripts/${script.systemId}.${baseExt}">${(script.name)!" "}</a>
		  				<#else>
		  					<a href="${articleBaseUrl + script.systemId?replace('_',replaceSpacesInUrlsWith,'r')}">${(script.name)!" "}</a>
		  				</#if>
					</td>
					<td class="default">${(script.systemId) !" "}</td>
					<td class="default">${(script.type)!" "}</td>
					<td class="default" style="white-space:normal;">${(script.description)!" "}</td>
					<td class="moreInfo">
						<#if fileBasedLinks>
		...
		function openModalInfo(ele){
			var frameUrl = '';
			<#if fileBasedLinks>
			frameUrl = "file:scripts/" + ele.parentElement.parentElement.parentElement.getAttribute("systemid") + ".${baseExt}"
